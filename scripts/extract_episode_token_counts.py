...
if __name__ == '__main__':
    ...
    sorted_episodes = sorted(os.listdir(subtitle_dir))
    ...
    for e in sorted_episodes:
        ...
        for d in e_data['dialogue']:
            clean_dialogue = clean_text(str(d)).replace('$', '')
        ...
        count_sums = list(pd.np.array(counts.sum(axis=1))[0])
    ...
    combined_counts_df.to_csv(os.path.join(out_dir, 'episode_token_counts.tsv'), sep='\t')
